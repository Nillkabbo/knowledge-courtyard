...
const SAVE_KEY = 'dlArchitect_v1';
const MAX_XP = 1000;
const RANKS = [
  {min:0,   name:'কোষাঙ্ক',          icon:'🧬'},
  {min:100, name:'স্নায়ু শিল্পী',     icon:'⚙️'},
  {min:200, name:'স্তর নির্মাতা',     icon:'🏗️'},
  {min:300, name:'দৃষ্টি প্রকৌশলী',   icon:'👁️'},
  {min:400, name:'ধারাবাহিকতার রক্ষক', icon:'🔗'},
  {min:500, name:'মনোযোগের স্থপতি',  icon:'⚡'},
  {min:600, name:'প্রতিমান নির্মাতা',  icon:'🎭'},
  {min:700, name:'চিন্তার স্থপতি',    icon:'🏛️'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=235+Math.random()*20;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

...
<div class="code-block"># ── STEP 1: Paper anatomy — IMRaD structure ──
...
# → Read first: is this relevant to your work?

# ABSTRACT (teaser, 150-250 words):
# → Problem + method + key result + implication
# → Read carefully: is the full paper worth reading?

# INTRODUCTION (why):
# → Wide context → narrow gap → contribution
# → Last paragraph = explicit contribution list

# METHODS (how):
# → Cookbook: architecture, hyperparameters, data
# → Should be detailed enough to reproduce independently

# RESULTS (what):
# → Figures/tables first, then numbers and interpretation
# → Baseline comparison + ablation studies essential

# DISCUSSION (so what):
# → Interpretation + limitations + future work
# → Never hide limitations (reviewers will find them)

# REFERENCES (chain):
# → Every claim has a citation; citation count = impact
...
reading_orders = {
    ...
    "PhD student": "Abstract → Related Work → Intro → future work",
    "Reviewer": "Abstract → Conclusion → Methods → Intro(last)",
}

for reader, order in reading_orders.items():
    print(f"  {reader}: {order}")

# FIGURES-FIRST RULE:
# Read ALL figures + captions BEFORE reading the text
# → "What story are they telling?"
# → Good paper: figures stand alone without text
# → Bad paper: you need text to understand figures

# PYTHON (paper reading tracker):
class PaperReader:
    """Track papers read and understanding level."""
    def __init__(self):
        self.papers = {}  # title → {reading_order, notes, rating, date}

    def read(self, title, reader_type="engineer", notes="", rating=3):
        self.papers[title] = {
            "order": reading_orders[reader_type],
            "notes": notes,
            "rating": rating,  # 1-5 stars
            "date": __import__("datetime").date.today(),
        }
        print(f"  Read: {title}")
        print(f"  Approach: {reading_orders[reader_type]}")

    def recommend_next(self, title):
        """Based on this paper's references, suggest what to read next."""
        print(f"  After {title}:")
        print(f"    → Check references for foundational papers")
        print(f"    → Search Google Scholar for citing papers (newer work)")

reader = PaperReader()
reader.read("Attention is All You Need (Vaswani et al., 2017)",
            "engineer", "Transformer architecture, self-attention", 5)</div>

<div class="code-block"># ── STEP 2: Engineer's paper note template ──
# How to take notes that actually help your work.

# ENGINEER'S PAPER NOTE TEMPLATE:
# (Don't write everything — write what MATTERS for your work)
...
1. PROBLEM (1-2 sentences):
   ...
   "Existing attention mechanisms are slow for long sequences."

2. KEY INSIGHT (1 sentence):
   The ONE idea that makes this paper work.
   "Self-attention can replace recurrence entirely — all positions
    computed in parallel."

3. METHOD (3-5 bullet points):
   Architecture / algorithm details (enough to understand, not reproduce).
   - Encoder-decoder structure
   - Multi-head self-attention (Q, K, V matrices)
   - Positional encoding (sinusoidal)
   - Layer normalization + residual connections

4. RESULTS (2-3 numbers):
   What did they achieve?
   - WMT 2014 English-to-German: 28.4 BLEU (2.0 above prior best)
   - 3.5B FLOPs per operation (efficient)
   - Trained on 8 P100 GPUs for 12 hours

5. CODE (link if available):
   - Official: github.com/tensorflow/tensor2tensor
   - HuggingFace: transformers library (BertModel, GPT2Model)

6. CAN I USE THIS? (1 sentence):
   How does this relate to YOUR work?
   "Use transformer encoder for our RAG retrieval model instead of LSTM."

7. LIMITATIONS (1-2 sentences):
   What doesn't this paper solve?
   "Quadratic memory for long sequences (O(n^2)).
    Fixed context window (no extrapolation beyond training length)."

8. FOLLOW-UP PAPERS:
   - "Reformer" (2020): efficient attention for long sequences
   - "Linformer" (2020): linear attention approximation
   - "Flash Attention" (2022): I/O-aware exact attention
"""

print(note_template)

# PAPER NOTE TIPS:
# → 1 page max (don't rewrite the paper)
# → Focus on what YOU need (engineer ≠ scientist)
# → Note the code link (you'll want to try it)
# → Connect to your work (how can you USE this?)
# → Rate understanding (1-5) → revisit low-rated papers later</div>

<div class="code-block"># ── STEP 3: Evaluating papers — trust but verify ──
# Not all papers are created equal.

# SCIENTIST VERIFICATION CHECKLIST (Trust Level 1-5):

verification = {
    "1. REPRODUCIBILITY": {
        "question": "Can I reproduce results with given info?",
        "trust_5": "Code + data + hyperparams all public",
        "trust_1": "No code, no data, vague methods",
    },
    "2. BASELINE COMPARISON": {
        "question": "Did they compare to state-of-the-art?",
        "trust_5": "Multiple recent SOTA baselines, fair comparison",
        "trust_1": "Old baselines or no comparison",
    },
    "3. STATISTICAL SIGNIFICANCE": {
        "question": "Are results significant or noise?",
        "trust_5": "Multiple runs, confidence intervals, p-values",
        "trust_1": "Single run, no error bars",
    },
    "4. PEER REVIEW": {
        "question": "Was this peer-reviewed?",
        "trust_5": "Top venue (NeurIPS, ICML, ACL, CVPR)",
        "trust_1": "arXiv preprint (not yet reviewed)",
    },
    "5. CITATION IMPACT": {
        "question": "Has the community built on this?",
        "trust_5": "1000+ citations, multiple follow-up papers",
        "trust_1": "0 citations, no follow-up",
    },
}

for criterion, details in verification.items():
    print(f"\n{criterion}")
    print(f"  Question: {details['question']}")
    print(f"  High trust: {details['trust_5']}")
    print(f"  Low trust: {details['trust_1']}")

# RED FLAGS (be skeptical):
red_flags = [
    "Cherry-picked examples (only showing best results)",
    "No ablation studies (each component's contribution unknown)",
    "Overfitting to benchmark (good on test set, bad in real world)",
    "Vague methods section (can't reproduce = can't verify)",
    "Selective baselines (comparing to weak old models)",
    "Exaggerated claims ('solves X forever')",
    "No failure analysis (what DOESN'T the method do?)",
    "Conflict of interest (corporate paper pushing their product)",
]
...
# → Tier 1 venues: NeurIPS, ICML, ICLR, ACL, EMNLP, CVPR, ICCV
# → Tier 2: AAAI, IJCAI, NAACL, COLING, ECCV
# → Workshops: lower bar, but cutting-edge ideas
# → arXiv: not peer-reviewed, but fastest dissemination
# → Always check: is code available? (huge trust signal)</div>

<div class="code-block"># ── STEP 4: Literature review and synthesis ──
# Building a map of a research area.

# HOW TO DO A LITERATURE REVIEW:

review_process = """
1. START WITH SURVEYS:
   → Find survey/review papers on your topic
   → Gives you the landscape (what exists, what's missing)
   → Search: "survey [topic] machine learning" on Google Scholar

2. FOLLOW THE CITATION CHAIN:
   → Forward: who cited this paper? (Google Scholar "Cited by")
   → Backward: what papers does this cite? (References section)
   → Build a tree of related work

3. IDENTIFY KEY PAPERS:
   → Papers cited by everyone (foundational)
   → Papers with most citations (impactful)
   → Papers winning best paper awards

4. ORGANIZE BY THEME:
   Group papers by approach:
   → "Approach A: RNN-based" (5 papers)
   → "Approach B: Attention-based" (3 papers)
   → "Approach C: Hybrid" (2 papers)
   → Compare: strengths, weaknesses, trade-offs

5. FIND THE GAP:
   → What has NOT been solved?
   → What are open problems mentioned in "future work"?
   → What limitations exist across ALL approaches?
   → This gap = your research opportunity
"""
...
tools = {
    "Google Scholar": "Free, comprehensive, citation tracking",
    "Semantic Scholar": "AI-powered paper recommendations",
    "Connected Papers": "Visual graph of related papers",
    "Research Rabbit": "Paper discovery + reference management",
    "Zotero / Mendeley": "Reference management + PDF organization",
    "Notion / Obsidian": "Notes connected to papers (Zettelkasten)",
    "arXiv Sanity": "arXiv paper recommendations by topic",
}
...
# PYTHON (literature graph):
class LiteratureGraph:
    """Track papers and their relationships."""
    def __init__(self):
        self.papers = {}  # title → {year, venue, cites}

    def add_paper(self, title, year, venue, references=None):
        self.papers[title] = {
            "year": year,
            "venue": venue,
    ...
    def most_cited(self):
        """Find papers cited by most others."""
        cite_count = {}
        for paper, data in self.papers.items():
            for ref in data["references"]:
                cite_count[ref] = cite_count.get(ref, 0) + 1
        return sorted(cite_count.items(), key=lambda x: x[1], reverse=True)

lg = LiteratureGraph()
lg.add_paper("BERT", 2018, "ACL", ["Attention is All You Need"])
lg.add_paper("GPT-2", 2019, "OpenAI", ["Attention is All You Need"])
lg.add_paper("T5", 2019, "NeurIPS", ["BERT", "Attention is All You Need"])

print("\nMost cited foundational papers:")
for paper, count in lg.most_cited():
    print(f"  {paper}: cited {count} times by our collection")</div>

<div class="code-block"># ── STEP 5: Writing your own paper ──
...
writing_process = """
1. IDEA PHASE:
   → Identify gap in literature (Door 4)
   → Formulate hypothesis ("Can X improve Y?")
   → Design experiment (how to test hypothesis)
   → Check novelty (has someone already done this?)

2. EXPERIMENT PHASE:
   → Implement method
   → Run experiments (multiple datasets, multiple seeds)
   → Compare with baselines (fair comparison!)
   → Ablation studies (each component's contribution)
   → Statistical significance (error bars, multiple runs)

3. WRITING PHASE:
   → Start with outline (Introduction → Methods → Results → Discussion)
   → Write Methods first (easiest, most concrete)
   → Write Results (figures + tables, then text)
   → Write Introduction (now you know what you're introducing)
   → Write Discussion (limitations, implications, future work)
   → Write Abstract LAST (summarize what you wrote)
   → Write Title LAST (reflect the actual contribution)

4. REVISION PHASE:
   → Self-review (read aloud, fix flow)
   → Co-author review (technical accuracy)
   → Peer review (submit → reviewer feedback → revise)
   → Camera-ready (final formatting, proofread)
"""

print(writing_process)

# PAPER WRITING TIPS:
tips = [
    "Tell a STORY (not just facts — narrative with tension + resolution)",
    "One idea per paragraph (if you can't summarize, it's too complex)",
    "Active voice > passive ('We trained' not 'was trained')",
    "Figures > tables > text (visual data is memorable)",
    "Consistent notation (don't use 'x' and 'X' for different things)",
    "Cite everything that isn't YOUR original idea",
    "Acknowledge limitations BEFORE reviewers find them",
    "Related work: organize by APPROACH, not chronologically",
    "Abstract: problem → method → result → impact (in 250 words)",
    "Proofread: typos signal carelessness → reviewers judge harder",
]
print("PAPER WRITING TIPS:")
for tip in tips:
    print(f"  ☐ {tip}")
...
print("\nTOP VENUES BY AREA:")
for area, venue_list in venues.items():
    print(f"  {area}: {venue_list}")</div>

<div class="code-block"># ── STEP 6: Research best practices and journey ──
# Becoming a researcher.

best_practices = [
    "Read 2-3 papers per week (consistent > binge)",
    "Take notes on every paper (template from Door 2)",
    "Follow top researchers on Twitter/GitHub/Scholar",
    "Attend conferences (or watch recorded talks)",
    "Join reading groups (discuss papers with peers)",
    "Reproduce key results (deepens understanding)",
    "Write blog posts about papers (teaching = learning)",
    "Start small (workshop paper → conference paper)",
    "Find a mentor (advisor, senior PhD, industry researcher)",
    "Embrace rejection (every researcher gets rejected)",
    "Collaborate broadly (different perspectives = better work)",
    "Manage references systematically (Zotero, Mendeley)",
    "Version control everything (Git for code AND papers)",
    "Present regularly (lab meetings, conferences, posters)",
    "Think about impact (what problem REALLY matters?)",
]
...
# │ Stage        │ Focus                                  │
# ├──────────────┼────────────────────────────────────────┤
# │ Beginner     │ Read papers, learn the landscape       │
# │              → Understand what exists, what's missing │
# ├──────────────┼────────────────────────────────────────┤
# │ Intermediate │ Reproduce papers, run experiments      │
# │              → Learn the tools, build intuition       │
# ├──────────────┼────────────────────────────────────────┤
# │ Advanced     │ Find novel problems, design solutions  │
# │              → First-author papers                    │
# ├──────────────┼────────────────────────────────────────┤
# │ Expert       │ Define research direction, mentor others│
# │              → Lead projects, program committees      │
# └──────────────┴────────────────────────────────────────┘
#
# "If I have seen further, it is by standing on the shoulders of giants."
#  — Isaac Newton
#
# Research is NOT about being the smartest.
# It's about being CURIOUS, PERSISTENT, and SYSTEMATIC.
# Read widely → Find gaps → Test ideas → Share results → Repeat.</div>

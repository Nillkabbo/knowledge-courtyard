...
Vue 3.5-নোট:
  ...
  senior: {
    ...
    body: `<p><strong>বাঁধন</strong>: <code>:</code> (attr/class/style; null→খসে); dynamic <code>:[name]</code>। <strong>ঘটনা</strong>: <code>@</code> + <code>.stop/.prevent/.capture/.self/.once/.passive</code> + key (<code>.enter</code>, <code>.ctrl.exact</code>) + mouse (<code>.left</code>)। <strong>শর্ত</strong>: v-if/else-if/else (মাউন্ট-টগল) বনাম v-show (display-টগল); একই উপাদানে v-if+v-for নিষেধ। <strong>তালিকা</strong>: v-for + <code>:key</code>(আসল id; index-নয়) — অ্যারে/অবজেক্ট/পরিসর/template-র‍্যাপ। <strong>একবার</strong>: v-once/v-memo/v-cloak। <strong>তালা</strong>: v-html কেবল trusted-উৎসে (LP: sanitize)। Vue 3.5: সব-শর্টহ্যান্ড + dynamic-arg বহাল।</p>`
  }
});

...
// Studio copy buttons (additive)
function copyStudio(btn){const pre=btn.closest('.studio-file').querySelector('pre');const text=pre.innerText;navigator.clipboard.writeText(text).then(()=>{btn.textContent='✓ কপি হয়েছে';setTimeout(()=>btn.textContent='📋 কপি',1600)}).catch(()=>{const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();try{document.execCommand('copy');btn.textContent='✓ কপি হয়েছে';setTimeout(()=>btn.textContent='📋 কপি',1600)}catch(e){}document.body.removeChild(ta)})}

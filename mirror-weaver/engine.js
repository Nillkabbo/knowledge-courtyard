...
// Map
const SECTIONS=[
[1,3,'০ · ভিত্তি','Foundation — the oath, the alphabet, the breath','#a5b4fc'],
[4,9,'I · কম্পোনেন্ট-কারুকাজ','Component Craft — props to composables','#6366f1'],
...
[22,27,'VI · স্টার্টার-কারখানা ও সমাপ্তি','Starter Factory & Finale — five looms, one mirror','#22d3ee']];
...
function updateHUD(){const pct=Math.min(100,(state.xp/MAX_XP)*100);document.getElementById('hud-xp-fill').style.width=pct+'%';document.getElementById('hud-xp-text').textContent=`${state.xp} / ${MAX_XP} XP`;let rank=RANKS[0];for(let i=RANKS.length-1;i>=0;i--){if(state.xp>=RANKS[i].min){rank=RANKS[i];break}}document.getElementById('hud-rank-icon').textContent=rank.icon;document.getElementById('hud-rank-name').textContent=rank.name;document.getElementById('hud-level').textContent=`স্তর ${String(Math.min(13,Math.floor(state.xp/100)+1)).replace(/[0-9]/g,d=>'০১২৩৪৫৬৭৮৯'[d])}`}
// Cheat Modal

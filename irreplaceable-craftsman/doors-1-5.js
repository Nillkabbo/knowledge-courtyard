...
<div class="code-block"># ── STEP 1: The perishability audit ──
# Which of your skills will survive the AI revolution?

# PERISHABILITY AUDIT:
# List ALL your skills → classify as Perishable (P) or Permanent (PE)

skills_audit = {
    # PERISHABLE (will be automated/devalued in 2-5 years):
    "React syntax": "P",
    "Django ORM tricks": "P",
    "AWS console navigation": "P",
    "Specific API knowledge": "P",
    "Framework-specific config": "P",
    "CSS layout tricks": "P",
    "IDE shortcuts": "P",

    # PERMANENT (valuable for decades):
    "Systems thinking": "PE",
    "First principles reasoning": "PE",
    "Communication": "PE",
    "Problem decomposition": "PE",
    "Pattern recognition": "PE",
    "Business understanding": "PE",
    "Curiosity / learning ability": "PE",
    "Debugging methodology": "PE",
    "Architecture / design": "PE",
    "Leadership / mentoring": "PE",
}

# Calculate your ratio:
permanent = sum(1 for v in skills_audit.values() if v == "PE")
total = len(skills_audit)
pct_permanent = permanent / total * 100

print(f"Total skills: {total}")
print(f"Permanent: {permanent} ({pct_permanent:.0f}%)")
print(f"Perishable: {total - permanent} ({100 - pct_permanent:.0f}%)")

if pct_permanent >= 70:
    print("✅ Healthy ratio — you're building durable value")
else:
    print("⚠️ Too much perishable! Invest in permanent skills")

# THE TEST: "Can AI do this in 2 years?"
# Yes → Perishable (in danger)
# No  → Permanent (safe)
# If your main skill vanished tomorrow, what remains?
# If "nothing" → you're in the obsolescence trap

# INVESTMENT STRATEGY:
# → 70% time on PERMANENT skills (compounds over decades)
# → 30% time on PERISHABLE skills (current job needs them)
# → Most juniors: 90% perishable → that's why they're replaceable</div>

<div class="code-block"># ── STEP 2: Deep work and first principles ──
# Building skills that compound.

# THE 5 WHYS (first principles decomposition):
# Ask "why" 5 times to find root cause/fundamental truth

def five_whys(problem):
    """Decompose a problem to root cause."""
    current = problem
    causes = [problem]
    for i in range(5):
        # In practice, you'd answer each "why" based on investigation
        current = f"WHY does this happen? → [investigate root cause]"
        causes.append(current)
    return causes

# Example: "Our app is slow"
# Why 1: Database queries take too long
# Why 2: N+1 query problem in Django ORM
# Why 3: No select_related() on the view
# Why 4: Developer didn't know about N+1
# Why 5: No code review process for database access
# ROOT CAUSE: Missing code review → fix the PROCESS, not just the code

# DEEP-WORK PORTFOLIO (invest time like money):
# Skill investment strategy:

portfolio = {
    # HIGH-RETURN, PERMANENT (invest most here):
    "Systems design": "Learned once, used forever (architecture, scaling)",
    "Data structures & algorithms": "Foundation for ALL coding",
    "Communication": "Writing, speaking, explaining → multiplier",
    "Learning how to learn": "Meta-skill → learn everything faster",
    "Problem decomposition": "Break complex problems into solvable parts",

    # MEDIUM-RETURN, SEMI-PERMANENT:
    "Cloud platforms (AWS/GCP)": "Concepts transfer, specifics change",
    "Database internals": "SQL/noSQL concepts are durable",
    "Security principles": "OWASP concepts persist for decades",
    "Testing methodology": "TDD, test pyramid — universal",

    # LOW-RETURN, PERISHABLE (invest least here):
    "Specific framework (React 19)": "Will be replaced in 3-5 years",
    "Specific tool (specific CI)": "Tools change constantly",
    "Syntax memorization": "Google/StackOverflow exists",
}

print("SKILL INVESTMENT PORTFOLIO:")
for skill, note in portfolio.items():
    print(f"  • {skill}: {note}")

# COMPOUND INTEREST OF SKILLS:
# → Frameworks: linear growth (learn new, forget old)
# → Permanent skills: COMPOUND (each year adds to previous)
# → After 10 years: framework-hopper = same value
#                   deep-skill-builder = 10x more valuable</div>

<div class="code-block"># ── STEP 3: The learning portfolio ──
# How to systematically build permanent skills.

# LEARNING PORTFOLIO (diversified skill investment):

learning_portfolio = {
    "TECHNICAL DEPTH (pick 1-2 areas to go deep)": [
        "Distributed systems (Kleppmann, Tanenbaum)",
        "Machine learning (math → architecture → deployment)",
        "Security (cryptography → network → application)",
        "Databases (internals, optimization, distributed)",
    ],
    "TECHNICAL BREADTH (know enough to connect ideas)": [
        "Cloud (AWS/GCP/Azure — concepts, not specifics)",
        "DevOps (Docker, K8s, CI/CD — enough to use)",
        "Frontend (React/Vue — enough to collaborate)",
        "Mobile (iOS/Android — enough to understand)",
    ],
    "META-SKILLS (multiply all other skills)": [
        "Writing (documentation, proposals, design docs)",
        "Public speaking (presentations, conferences)",
        "Negotiation (salary, scope, deadlines)",
        "Teaching (mentoring, blogging, courses)",
    ],
    "DOMAIN KNOWLEDGE (industry expertise)": [
        "Finance (for fintech roles)",
        "Healthcare (for medtech roles)",
        "Education (for edtech roles)",
        "Your specific industry → moat against AI",
    ],
}

for category, skills in learning_portfolio.items():
    print(f"\n{category}:")
    for skill in skills:
        print(f"  • {skill}")

# THE T-SHAPED ENGINEER:
# → Deep in 1-2 areas (vertical bar of T)
# → Broad across many areas (horizontal bar of T)
# → Broad + shallow = replaceable (AI can do general)
# → Deep + narrow = pigeonholed (limited opportunities)
# → T-shaped = irreplaceable (depth + breadth + connections)

# LEARNING RATE FORMULA:
# Learning speed = (study_time × focus_quality × method_quality)
# → Study time: obvious (more time = more learning)
# → Focus quality: deep work >> distracted work
# → Method quality: active recall + spaced repetition >> re-reading</div>

<div class="code-block"># ── STEP 4: Design critique and craftsmanship ──
# The mindset of an irreplaceable engineer.

# DESIGN CRITIQUE CHECKLIST (review your own work):
critique = [
    "Does this solve the REAL problem? (not just the stated one)",
    "Is this the SIMPLEST solution that works? (Occam's razor)",
    "What happens when this fails? (error handling, edge cases)",
    "How will this scale 10x? (architecture flexibility)",
    "Can a new team member understand this in 10 minutes?",
    "Is this tested? (unit, integration, edge cases)",
    "Is this secure? (input validation, auth, injection)",
    "Is this observable? (logging, metrics, tracing)",
    "Is this documented? (README, comments, API docs)",
    "Would I be proud to show this in an interview?",
]

print("DESIGN CRITIQUE CHECKLIST:")
...
1. OBSESSION WITH QUALITY:
   ...
   → Every commit is a signature
   → Code is read 10x more than written — optimize for readers

2. CURIOSITY DRIVES DEPTH:
   → Don't just use a tool — understand HOW it works
   → Read source code of libraries you use
   → Ask "why" until you hit fundamentals

3. OWNERSHIP:
   → "Not my job" = career-limiting phrase
   → Own the problem end-to-end (not just your code)
   → Production issues = YOUR issues (not "ops problem")

4. ANTICIPATION:
   → Think 2 steps ahead (what will break? what will change?)
   → Build for tomorrow, not just today
   → But don't over-engineer (YAGNI)

5. FEEDBACK LOOPS:
   → Seek criticism (not praise)
   → Code reviews are learning opportunities
   → Post-mortems without blame → growth culture
"""

print(mindset)

# IRREPLACEABLE = PROBLEM SOLVER, NOT CODE WRITER
# AI can write code → the code writer is replaceable
# AI can't yet: understand business context, navigate politics,
# make trade-off decisions, mentor humans, define problems
# → Build skills AI CAN'T replicate</div>

<div class="code-block"># ── STEP 5: Storytelling and visibility ──
# Being good isn't enough — people need to KNOW you're good.

# TECH STORY TEMPLATE (communicate your work effectively):
# → Hiring managers and leaders think in STORIES, not facts

story_template = """
TECHNICAL STORY FORMAT:

HOOK (1 sentence): What was the problem?
  "Our payment API was timing out for 5K users during peak hours."

SITUATION (2-3 sentences): Context and stakes
  "It was tax season. Every minute of downtime cost $10K in lost
   transactions. The team had tried 3 fixes, none worked."

ACTION (3-5 sentences): What YOU did (technical detail)
  "I profiled the system with Datadog and found N+1 queries.
   I refactored to use select_related(), added Redis caching,
   and implemented connection pooling."

RESULT (2-3 sentences): Measurable impact
  "Response time dropped from 3s to 200ms. Zero timeouts in
   the next 3 months. The fix handled 50K requests during peak.
   I also added load tests to CI/CD to prevent regression."

LESSON (1 sentence): What you learned
  "I learned that profiling before optimizing saves days of guessing."
"""

print(story_template)

# VISIBILITY STRATEGIES (build reputation):
visibility = {
    "Write": "Blog posts, design docs, internal wikis",
    "Speak": "Conference talks, meetups, internal presentations",
    "Open Source": "Contribute to libraries you use",
    "Teach": "Mentor juniors, create courses, workshops",
    "Document": "Make knowledge accessible (others will credit you)",
    "Network": "Build relationships beyond your immediate team",
    "Lead": "Drive initiatives, not just tasks",
}

print("VISIBILITY STRATEGIES:")
for strategy, action in visibility.items():
    print(f"  • {strategy}: {action}")

# THE UNCOMFORTABLE TRUTH:
# → Your work doesn't speak for itself (you must speak for it)
# → "Humble" engineers get passed over for promotions
# → Visibility is not bragging — it's ensuring your impact is known
# → Quiet excellence + strategic visibility = career acceleration</div>

<div class="code-block"># ── STEP 6: Career durability and best practices ──
# How to stay relevant for decades.

best_practices = [
    "Invest 70% in permanent skills (systems, communication, thinking)",
    "Go deep in 1-2 areas (T-shaped engineer)",
    "Stay broad across the stack (enough to connect ideas)",
    "Read source code, not just documentation",
    "Build things end-to-end (not just your piece)",
    "Write about what you learn (blog, docs, wiki)",
    "Teach others (mentoring cements your own knowledge)",
    "Seek feedback actively (code reviews, 1-on-1s)",
    "Understand the business (not just the tech)",
    "Network authentically (relationships > transactions)",
    "Save 6 months expenses (freedom to make bold moves)",
    "Embrace AI tools (they multiply your output, don't fear them)",
    "Take calculated risks (new project, new role, new company)",
    "Protect your reputation (it takes years to build, seconds to destroy)",
    "Think in decades, not quarters (compounding requires patience)",
]

print("CAREER DURABILITY BEST PRACTICES:")
...
# ├──────────────────┼──────────────────────────────────┤
# │ DEPTH            │ Expert-level knowledge in 1-2  │
# │                  │ areas that AI can't replicate  │
# ├──────────────────┼──────────────────────────────────┤
# │ BREADTH          │ Enough knowledge across the    │
# │                  │ stack to connect ideas         │
# ├──────────────────┼──────────────────────────────────┤
# │ JUDGMENT         │ Business context, trade-offs,  │
# │                  │ leadership, human dynamics     │
# └──────────────────┴──────────────────────────────────┘
#
# Depth + Breadth + Judgment = IRREPLACEABLE
# Any one alone = replaceable
# All three = career moat that compounds over decades</div>`,
  senior:{

// ════════════════════════════════════════
// Book 32 · Brain Mechanic — Engine
// ════════════════════════════════════════
let state = { xp:0, currentDoor:null, completedDoors:[], prologueSeen:false };
const XP_PER_DOOR = 100;
const SAVE_KEY = 'mindMechanic_v1';
const MAX_XP = 1500;
const RANKS = [
  {min:0,    name:'ঘুমন্ত মন',     icon:'😴'},
  {min:100,  name:'জেগে ওঠা',      icon:'🌅'},
  {min:200,  name:'অভ্যাস চাষি',   icon:'🌱'},
  {min:300,  name:'ফোকাস শিল্পী',  icon:'🎯'},
  {min:400,  name:'মন প্রকৌশলী',   icon:'⚙️'},
  {min:500,  name:'অভ্যাস স্থপতি',  icon:'🏛️'},
  {min:600,  name:'নিউরো-কারিগর',  icon:'🔬'},
  {min:700,  name:'মস্তিষ্কের কারিগর', icon:'🧠'},
  {min:1000, name:'আত্ম-নিয়ন্ত্রণের কেন্দ্র', icon:'👑'},
  {min:1500, name:'মস্তিষ্কের সম্রাট', icon:'⚡'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=280+Math.random()*40;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

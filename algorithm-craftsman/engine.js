...
const SAVE_KEY = 'algorithmCraftsman_v1';
const MAX_XP = 2100;
const RANKS = [
  {min:0,    name:'শিক্ষানবিশ কারিগর',     icon:'🛖'},
  {min:100,  name:'সহকারী',                 icon:'🔨'},
  {min:300,  name:'কোড নির্মাতা',           icon:'⚙️'},
  {min:500,  name:'অ্যালগরিদম বোঝে',        icon:'🧩'},
  {min:700,  name:'ইনডেক্স কারিগর',         icon:'📇'},
  {min:900,  name:'অনুসন্ধানী',             icon:'🔍'},
  {min:1100, name:'সার্চ প্রকৌশলী',         icon:'🚀'},
  {min:1300, name:'সিস্টেম স্থপতি',         icon:'🏗️'},
  {min:1600, name:'মাস্টার কারিগর',         icon:'👑'},
  {min:1900, name:'জ্ঞানের কারিগর',         icon:'⭐'},
  {min:2100, name:'সর্বোচ্চ নির্মাতা',       icon:'🌟'},
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=15+Math.random()*25;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

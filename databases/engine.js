// ══════════════════════════════════════
// Book 39 · Databases (Expanded v2) — Engine
// ══════════════════════════════════════
let state = { xp:0, currentDoor:null, completedDoors:[], prologueSeen:false };
const XP_PER_DOOR = 100;
const SAVE_KEY = 'dataVault_v2';
const MAX_XP = 1300;
const RANKS = [
  {min:0,    name:'উপাত্ত শিক্ষার্থী',     icon:'📋'},
  {min:100,  name:'সারি সংগঠক',         icon:'📊'},
  {min:200,  name:'সূচক নির্মাতা',         icon:'🗂️'},
  {min:300,  name:'লেনদেন প্রহরী',        icon:'🔐'},
  {min:400,  name:'অমৃতত্ব কারিগর',       icon:'∞'},
  {min:500,  name:'বিতরণ স্থপতি',         icon:'🌐'},
  {min:600,  name:'অপ্টিমাইজার',          icon:'⚡'},
  {min:700,  name:'উপাত্তের গুরু',        icon:'🏛️'},
  {min:900,  name:'ভেক্টর স্থপতি',        icon:'🧭'},
  {min:1100, name:'AI উপাত্ত কারিগর',     icon:'🤖'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=160+Math.random()*15;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

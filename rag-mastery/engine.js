// ════════════════════════════════════════
// Book 10 · RAG Mastery — Engine
// ════════════════════════════════════════
let state = { xp:0, currentDoor:null, completedDoors:[], prologueSeen:false };
const XP_PER_DOOR = 100;
const SAVE_KEY = 'ragMastery_v1';
const MAX_XP = 1000;
const RANKS = [
  {min:0,   name:'পানি সংগ্রাহক',     icon:'🪣'},
  {min:100, name:'কূপ খনক',   icon:'📚'},
  {min:200, name:'জল সংরক্ষক',     icon:'✂️'},
  {min:300, name:'নালা নির্মাতা',     icon:'🔍'},
  {min:400, name:'কুয়ো স্থপতি',           icon:'🧪'},
  {min:500, name:'জ্ঞান কূপ',     icon:'🔗'},
  {min:600, name:'কুয়ো স্থপতি',    icon:'🌊'},
  {min:700, name:'অফুরন্ত কূপ',    icon:'👑'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=195+Math.random()*25;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

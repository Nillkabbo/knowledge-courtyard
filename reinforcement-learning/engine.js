...
const RANKS = [
  ...
  {min:100, name:'মাটি-চেনা বাগানি',    icon:'🧤'},
  {min:200, name:'ধৈর্যশীল চাষি',       icon:'🌾'},
  {min:300, name:'ফলন-হিসাবি',          icon:'📊'},
  {min:400, name:'ঋতু-জানা কৃষক',      icon:'🌦️'},
  {min:500, name:'বাগান-স্থপতি',        icon:'🏡'},
  {min:600, name:'মাঠ-পরীক্ষক',         icon:'🧪'},
  {min:700, name:'নীতি-নির্ধারক',       icon:'🧭'},
  {min:800, name:'পুরস্কার-কারিগর',     icon:'🏆'},
  {min:900, name:'উদ্যান-রক্ষক',        icon:'🌳'},
  {min:1000,name:'বাগানের ওস্তাদ',      icon:'👑'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=120+Math.random()*30;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

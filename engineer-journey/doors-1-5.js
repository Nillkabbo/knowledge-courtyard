...
<div class="code-block"># ── STEP 1: First principles in API design ──
# Thinking from fundamentals, not copying patterns.

# ❌ JUNIOR: "This is how my previous project did it"
class UserAPI:
    def get_all_users(self, include_deleted,
                      force_refresh, legacy_mode,
                      debug_output, skip_cache):
        # 6 parameters — nobody knows what each does
        pass

# ✅ SENIOR: Fundamental question — "What does the user NEED?"
# Answer: A list of users, with some filtering
class UserAPI:
    ...
    def get_user(self, user_id: str) -> User:
        ...
        pass
# 2 clear methods. Because the problem was decomposed properly.

# FIRST PRINCIPLES QUESTIONS (ask before every task):
# 1. What IS the problem? (not the symptom)
# 2. Why does this matter? (business value)
# 3. What's the simplest solution that works? (Occam's razor)

# PYTHON (good vs bad API design):
...
class UserService:
    def list_users(self, filters: UserFilters) -> list[User]:
        ...
        query = User.objects.filter(is_active=filters.active)
        if filters.role:
            query = query.filter(role=filters.role)
        return query[:filters.limit]

    def get_user(self, user_id: str) -> User:
        """Get single user or raise NotFound."""
        return User.objects.get(id=user_id)

    def create_user(self, email: str, name: str) -> User:
        """Create user. Raises if email exists."""
        if User.objects.filter(email=email).exists():
            raise ValueError("Email already registered")
        return User.objects.create(email=email, name=name)

# API DESIGN PRINCIPLES:
# → Clear naming (list_users, not get_all_users_with_options)
# → Few parameters (use objects for complex params)
# → Return types are explicit and consistent
# → Error handling is predictable (raise specific exceptions)
# → Single responsibility (each method does ONE thing)</div>

<div class="code-block"># ── STEP 2: Problem decomposition ──
# Breaking complex problems into manageable parts.

# E-COMMERCE SYSTEM DECOMPOSITION:

# Don't build "an e-commerce system"
# Build: auth → catalog → cart → checkout → payment → shipping → tracking

components = {
    "Auth Service": {
        "responsibility": "User registration, login, tokens",
        "endpoints": ["/register", "/login", "/refresh", "/logout"],
        "tech": "JWT + Redis for session",
    },
    "Product Catalog": {
        "responsibility": "Product CRUD, search, categories",
        "endpoints": ["/products", "/products/{id}", "/search"],
        "tech": "PostgreSQL + Elasticsearch",
    },
    "Cart Service": {
        "responsibility": "Add/remove items, calculate totals",
        "endpoints": ["/cart", "/cart/items", "/cart/checkout"],
        "tech": "Redis (fast, ephemeral)",
    },
    "Order Service": {
        "responsibility": "Create orders, track status",
        "endpoints": ["/orders", "/orders/{id}"],
        "tech": "PostgreSQL (durable, ACID)",
    },
    "Payment Service": {
        "responsibility": "Process payments, refunds",
        "endpoints": ["/payments", "/payments/{id}/refund"],
        "tech": "Stripe API + idempotency keys",
    },
    "Notification Service": {
        "responsibility": "Email/SMS/push notifications",
        "endpoints": ["/notify"],
        "tech": "Celery + SendGrid/Twilio",
    },
}

print("E-COMMERCE DECOMPOSITION:")
...
# 2. Clear boundaries: services communicate via APIs, not shared DB
# 3. Independent deployment: update one without breaking others
# 4. Independent scaling: scale payment service more during sales
# 5. Fault isolation: if cart fails, catalog still works

# WHEN TO DECOMPOSE:
# → Start monolithic (simpler)
# → Decompose when: team > 8 people, deployment conflicts, scaling needs
# → Premature microservices = distributed monolith (worst of both worlds)</div>

<div class="code-block"># ── STEP 3: Abstraction — good vs bad ──
# The art of hiding complexity.

# GOOD ABSTRACTION: hides complexity, reveals simplicity
# Django ORM: you write User.objects.filter(name="Rakib")
# → You don't think about SQL, indexes, connection pooling
# → The abstraction handles ALL of that

# BAD ABSTRACTION: leaks complexity, adds confusion
# Framework that requires 5 config files to print "hello"
# → Abstraction adds complexity instead of hiding it

# PYTHON (abstraction examples):
...
class PaymentProcessor:
    ...
    def charge(self, amount: float, currency: str = "USD") -> str:
        ...
        # Internally: validate, convert currency, call Stripe,
        # handle retries, log, audit — ALL hidden from caller
        transaction = self._call_stripe(amount, currency)
        self._log_transaction(transaction)
        return transaction.id

# ❌ BAD: Leaky abstraction (caller must know internals)
class BadPaymentProcessor:
    def charge(self, amount, currency, retry_count,
               timeout, api_key, webhook_url,
               idempotency_key, merchant_id):
        # Caller needs to know about retries, timeouts, webhooks
        # This isn't an abstraction — it's a config nightmare
        pass

# ABSTRACTION LEVELS (from high to low):
# 1. Product (user sees: "Buy now" button)
# 2. API (developer sees: POST /payments)
# 3. Service (internal: PaymentProcessor.charge())
# 4. Infrastructure (Stripe API call)
# 5. Network (HTTPS request/response)
# 6. OS (sockets, TCP/IP)
# 7. Hardware (electrical signals)

# Each layer abstracts the one below. You don't need to understand
# TCP/IP to build a payment system. That's the power of abstraction.

# THE ABSTRACTION RULE:
# → Make common things EASY (95% of use cases: one-liner)
# → Make complex things POSSIBLE (5%: escape hatches available)
# → Don't make common things require understanding internals</div>

<div class="code-block"># ── STEP 4: Engineering trade-offs ──
# Every decision has costs. Choose wisely.

# SOFTWARE ENGINEERING TRADE-OFFS:

tradeoffs = """
1. CONSISTENCY vs AVAILABILITY (CAP Theorem):
   → Can't have both during network partition
   → Banking: choose consistency (no wrong transactions)
   → Social media: choose availability (feed always loads)

2. LATENCY vs COST:
   → CDN everywhere = fast but expensive
   → Single region = cheap but slow for distant users
   → Cache everything = fast but stale data risk

3. SIMPLICITY vs FLEXIBILITY:
   → Simple system: easy to understand, hard to extend
   → Flexible system: easy to extend, hard to understand
   → Rule: start simple, add flexibility when NEEDED (YAGNI)

4. NORMALIZATION vs DENORMALIZATION:
   → Normalized: no duplicate data, but slow joins
   → Denormalized: fast reads, but data sync complexity
   → Rule: normalize for OLTP, denormalize for OLAP

5. SYNCHRONOUS vs ASYNCHRONOUS:
   → Sync: simpler, but caller waits (timeout risk)
   → Async: faster response, but complexity (queues, retries)
   → Rule: sync for UI, async for background work

6. BUILD vs BUY:
   → Build: full control, but maintenance burden
   → Buy: fast setup, but vendor lock-in + cost
   → Rule: buy commodity, build differentiation

7. MONOLITH vs MICROSERVICES:
   → Monolith: simple deploy, but scaling limits
   → Microservices: independent scaling, but operational complexity
   → Rule: start monolith, extract services when pain is real
"""
...
# → Don't ask "which is best?" → ask "which trade-off fits OUR context?"
# → Document the trade-off (ADR: Architecture Decision Record)
# → Revisit when context changes (what was right yesterday may be wrong today)</div>

<div class="code-block"># ── STEP 5: Resilience patterns ──
# Building systems that survive failures.

# RESILIENCE PATTERNS (production-grade code):

resilience = """
1. CIRCUIT BREAKER:
   → If service fails N times → stop calling (open circuit)
   ...
   def call_payment_service():
       return requests.post("https://payment.api/charge")
   # After 5 failures: stops calling for 30 seconds

2. RETRY WITH EXPONENTIAL BACKOFF:
   → First failure: wait 1s, retry
   → Second failure: wait 2s, retry
   → Third failure: wait 4s, retry
   → Then: give up (or queue for later)
...
3. BULKHEAD:
   → Isolate resources (don't let one failure sink everything)
   → Separate thread pools for different services
   → If payment calls fail → cart still works

4. TIMEOUT:
   → NEVER call external service without timeout
   → Default: 5-10 seconds for API calls
   → Faster fail → faster recovery

5. RATE LIMITING:
   → Protect your service from being overwhelmed
   → Token bucket: N requests per second
   → Return 429 (Too Many Requests) when exceeded

6. IDEMPOTENCY:
   → Same operation → same result (safe to retry)
   → Idempotency key: unique ID per operation
   → Critical for payments (don't charge twice!)

7. GRACEFUL DEGRADATION:
   → If recommendation service fails → show popular items
   → If search fails → show categories
   → If analytics fails → still process orders
   → Core function works, extras degrade gracefully
"""
...
# ☐ Alerting (PagerDuty, Slack)</div>

<div class="code-block"># ── STEP 6: Senior engineer mindset and best practices ──
# How senior engineers think differently.

best_practices = [
    "Ask 'why' 5 times before writing code (find root cause)",
    "Design for failure (what breaks? how do we recover?)",
    "Simple > clever (readable code wins long-term)",
    "Document trade-offs (ADR: Architecture Decision Records)",
    "Optimize for the reader, not the writer",
    "Make common operations easy, complex possible",
    "Own production (it's YOUR code in production)",
    "Monitor before optimizing (measure, don't guess)",
    "Automate toil (CI/CD, testing, deployment)",
    "Review every PR (yours and others')",
    "Write post-mortems (blameless, learn from failures)",
    "Mentor juniors (teaching cements your knowledge)",
    "Read code (more than you write — especially good code)",
    "Understand the business (why does this matter to users?)",
    "Think in systems (how does your piece fit the whole?)",
]

print("SENIOR ENGINEER BEST PRACTICES:")
for practice in best_practices:
    print(f"  ☐ {practice}")

# THE JOURNEY: Junior → Mid → Senior
# ┌──────────┬──────────────────────────────────────────┐
# │ Level    │ Mindset                                  │
# ├──────────┼──────────────────────────────────────────┤
# │ Junior   │ "How do I write this code?"              │
# │          → Focus: syntax, tools, completing tasks  │
# ├──────────┼──────────────────────────────────────────┤
# │ Mid      │ "Why are we building this?"              │
# │          → Focus: architecture, trade-offs, quality │
# ├──────────┼──────────────────────────────────────────┤
# │ Senior   │ "What problem are we REALLY solving?"    │
# │          → Focus: systems, business, people, impact │
# ├──────────┼──────────────────────────────────────────┤
# │ Staff+   │ "What problems SHOULD we be solving?"    │
# │          → Focus: strategy, org, alignment, vision  │
# └──────────┴──────────────────────────────────────────┘
#
# The transition isn't about coding faster.
# It's about thinking bigger, deeper, and longer-term.</div>

...
const RANKS = [
  ...
  {min:300,  name:'নদী পাহারাদার',         icon:'🌊'},
  {min:500,  name:'রন্ধনশিল্পী',           icon:'🍳'},
  {min:700,  name:'ভাণ্ডার রক্ষক',         icon:'📦'},
  {min:900,  name:'কারিগর',               icon:'🔧'},
  {min:1100, name:'নকশাবিদ',              icon:'🏗️'},
  {min:1300, name:'স্থপতি',               icon:'🏛️'},
  {min:1500, name:'দক্ষ কারিগর',          icon:'💎'},
  {min:1800, name:'মাস্টার বিল্ডার',       icon:'👑'}
];
...
// Cheat Modal
function openCheatModal(){const grid=document.getElementById('cheat-modal-grid');const empty=document.getElementById('cheat-modal-empty');empty.textContent='';grid.innerHTML=doors.map((d,i)=>{const done=state.completedDoors.includes(i);return `<div class="cheat-card${done?' completed':''}"><div class="icon">${d.icon}</div><div class="name">${done?'✅ ':''}${d.name}</div><div class="tech">${d.tech}</div><div class="secret">${d.secret}</div></div>`}).join('');document.getElementById('cheat-modal').style.display='block'}
// Door/Story

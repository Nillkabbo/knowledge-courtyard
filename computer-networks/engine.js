...
const SAVE_KEY = 'networksWeb_v1';
const MAX_XP = 1000;
const RANKS = [
  {min:0,   name:'সংকেত পাঠক',       icon:'📡'},
  {min:100, name:'তারের যোদ্ধা',     icon:'🔌'},
  {min:200, name:'রাস্তার প্রহরী',    icon:'🛣️'},
  {min:300, name:'নামের রক্ষক',      icon:'📖'},
  {min:400, name:'বার্তাবাহক',       icon:'✉️'},
  {min:500, name:'সুরক্ষার কারিগর',   icon:'🛡️'},
  {min:600, name:'নেটওয়ার্ক স্থপতি', icon:'🏗️'},
  {min:700, name:'সংযোগের গুরু',    icon:'🌐'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=195+Math.random()*15;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

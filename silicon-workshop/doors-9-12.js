...
<div class="dialogue en">Sharmin opened an empty file. "Write. No fear — the compiler will shout, not the chip." You typed the first line — DDRB |= (1<<5); — compile, flash, and... nothing. "Of course! You gave the costume, not the command." You added PORTB ^= (1<<5); — and a strange glow: the LED toggled so fast your eye merged it into dim light! Sharmin laughed: "Welcome to reality — in a 16MHz city your 'one line' runs a hundred thousand times a second. You'll need a delay-letter — that's the timer-door's story; for now, _delay_ms()." You added the delay — and for the first time saw it: your rhythm, your LED. Kun fa-yakun. You will not forget that moment.</div>
...
<div class="dialogue en">Muaz turned the datasheet to the ladder row — 1, 8, 64, 256, 1024. "Choosing the drum-division is setting the ceiling: the fast drum overflows the counter in 4ms; divided by 256, seconds fit in your palm. You calculate — I stay quiet." You thought, wrote: 16,000,000 ÷ 256 = 62,500. "Now?" "One less — 62,499!" — "Why one less?" — "Counting... from zero!" He smiled — his father's shadow in it: "That's an accountant's page. Now seat it in code." You wrote OCR1A=62499; compile, flash; the speaker toned exactly each second — matching the RTC's light, letter for letter. Two clocks, two worlds — one heartbeat.</div>
...
RTC (ক্যালেন্ডার-হৃদয়):
  ৩২.৭৬৮kHz ক্রিস্টাল = ২^১৫ — ১৫-ভাগে ঠিক ১Hz!
...
doors.push({
  ...
  recall:{
    ...
    a:"PWM (Pulse-Width Modulation): পিন সবসময় পুরো HIGH বা পুরো LOW — কিন্তু সময়-ভাগে মাপা: duty cycle = HIGH-সময়ের ভগ্নাংশ। ৫০% duty মানে প্রতি পর্যায়ে অর্ধেক সময় পুরো-চালু, অর্ধেক পুরো-বন্ধ — LED/মোটর এত দ্রুত দোলে যে চোখ/জড়তা গড় অনুভব করে: গড় শক্তি ≈ duty × পুরো। তাই 'অর্ধেক আলো' = ৫০%-HIGH-সময়, ভোল্টেজ-অর্ধেক নয় (দ্রুত-সুইচিং ডিজিটালে লিনিয়ার-অ্যানালগের কাজ; ট্রানজিস্টর স্যাচুরেশনে দক্ষ, তাপ-সাশ্রয়ী)। মোটরে একই: জড়তা (Door ২-এর L!) গড় মেনে নেয়; বাজারের servo ঠিক এই পালস-প্রস্থ পড়ে (১ms-২ms) কোণ ঠিক করে। ADC (Analog-to-Digital Converter): উল্টো সেতু — ভোল্টেজকে সংখ্যায়। ১০-বিট = ২^১০=১০২৪ ধাপ; Vref=৩.৩V হলে ধাপ = ৩.৩/১০২৪ ≈ ৩.২২mV। ২.৫V → ২.৫÷৩.২২mV ≈ ৭৭৬ (আসলে ২.৫/৩.৩×১০২৩ ≈ ৭৭৫)। রেজোলিউশন ধাপ-সংখ্যা (৮/১০/১২-বিট), Vref মাপ-কাঠির পুরো দৈর্ঘ্য। Nyquist-সতর্কতা: স্যাম্পল-হার সংকেতের সর্বোচ্চ কম্পনাংকের অন্তত দ্বিগুণ, নইলে ভুল-ছদ্মবেশ (aliasing)। ADC-পাঠে সবসময় ডিভাইডার-সেতু (Door ৬: LDR+রেজিস্টর) মনে রেখো — সেন্সর-রেজিস্টর বদলায়, ডিভাইডার ভোল্টেজ বদলায়, ADC সংখ্যায় পড়ে।",

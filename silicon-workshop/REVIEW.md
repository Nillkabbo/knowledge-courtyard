...
>
> **Reader-experience pass** — read all 16 doors end-to-end as continuous prose (plaintext extraction, since live browser rendering wasn't available in-sandbox). Found and fixed two defects invisible to the mechanical audit: Door 3 "আজরের আগে" → "আসরের আগে" (misspelled "before Asr"), and a corrupted glyph in Door 16's "পুনরাবৃত্তি" (repeatability). No other issues in the remaining 14 doors.
>
> **Technical accuracy audit** — four parallel deep-dive passes (one per door file) re-verified every formula, every named component/IC spec, every AVR/Arduino/ESP-IDF code snippet, and every historical/dated claim against real datasheets and web sources, independent of the original review. Confirmed and fixed 8 genuine issues: Door 3's forced-β sidebar was internally contradictory (2.6mA base claimed to both need only "26mA is enough" and separately drive a 100mA load — corrected to state the achieved forced-β≈38, consistent with the diagram's own 100mA/2.6mA figures); Door 4's radio duty-cycle example said "every 15 minutes" while its own 0.5mA result and the English text both required 10 minutes; Door 4's duty-cycle savings figure said "80×" where the diagram two paragraphs later correctly says "83×"; Door 6's soil-probe worked example had the wet-soil resistance off by 10× (300Ω instead of ~3kΩ to actually produce the stated 0.76V); Door 7 had Shannon's age wrong (said 20, he was 21 when he wrote the 1937 MIT thesis — both languages fixed); Door 11's timer recall answer had a garbled, self-contradicting toggle-frequency aside (claimed 31.25kHz for a 1Hz compare-match, corrected to the actual 0.5Hz); Door 14 claimed the ARM vector table's *first* entry is the reset vector (it's actually the initial stack-pointer value — the reset handler is the second entry — this also contradicts the door's own later code-block); Door 15's English recall answer ambiguously juxtaposed "OASIS 2014/ISO 20922" as if both standards dated to 2014 (ISO/IEC 20922 was actually 2016 — clarified). A handful of lower-confidence items (Door 3's MOSFET/Shockley-1949 lineage phrasing, Door 8's AVR "1996" design-vs-1997-shipped nuance, Door 13's I2C 1980-vs-1982 birth year) were left as-is: defensible simplifications, not confirmed errors. `node --check` clean on all four files post-fix; `audit.py` unchanged (still only the 9 confirmed-intentional structural deviations).
>
> **Live-browser visual audit** — every prior visual check had been static (cairosvg rasterisation or plain-text extraction); this pass got a real Chromium engine running against the actual `index.html` for the first time. No display was available, so a real headless-browser stack was built from scratch in-sandbox: `pip install playwright`, downloaded Chromium successfully, but launch failed on one missing OS shared library (`libXdamage.so.1`) with no root/apt access available to install it. Fixed by compiling a 4-symbol stub `.so` (`XDamageCreate/Destroy/QueryExtension/Subtract`, all no-ops — the real extension is never needed for screenshot rendering) and pointing `LD_LIBRARY_PATH` at it; Chromium then launched clean. Bengali glyphs are unrenderable in this sandbox (no Bengali font installable — network egress is blocked at the sandbox proxy for both `apt` and direct `curl`/`pip`-adjacent font CDNs; confirmed by testing `fonts-droid-fallback` and scanning all 394 installed fonts for Bengali coverage — zero found), so Bengali text renders as tofu boxes in these screenshots; English text, diagram geometry, wiring, colour, layout, and photos are all fully legible and were the actual audit target.
>
> Method: `state.completedDoors` set via `page.evaluate` to unlock all 16 doors, then `openDoor(idx)` called for each in turn on the live app (not a static file dump — real JS state machine, real CSS cascade, real image loading). Automated checks per door: horizontal overflow (`scrollWidth` vs `clientWidth`), broken `<img>` (`naturalWidth===0`), zero-size SVG diagrams, and browser console errors/warnings. Result: **zero** on all four counts, across all 16 doors. Every `.diagram` element (55 total) and `.photo-frame` (16 total, one per door) was then screenshotted individually and reviewed — all 55 diagrams render with correct non-zero geometry, correct wire routing, correctly shaped symbols (logic gates with proper NAND bubbles, bridge rectifier's 4-diode diamond, byte-level DDRB/PORTB bit-box highlighting, I²C multi-drop bus topology, MQTT broker fan-in/fan-out, etc.), and no overlapping or clipped elements; all 16 real photos loaded and displayed correctly.
>
> One false positive investigated to ground: Door 15's antenna photo showed a dark bar with placeholder text across its top ~5% in the screenshots. Traced to Playwright's `chromium_headless_shell` (a stripped testing-only binary) mis-painting that specific JPEG's embedded 672-byte ICC colour profile — confirmed by (a) the raw file rendering clean in a normal image viewer, (b) the artifact surviving an explicit `img.decode()` wait (ruling out a load-timing race), (c) the artifact disappearing when the ICC profile was stripped and the image re-saved, and (d) most conclusively, the *original, unmodified* file rendering perfectly clean under Playwright's **full** Chromium binary (`chromium-1234`, not `chromium_headless_shell`) — the build far closer to a real desktop Chrome. Verdict: a quirk of the minimal test binary, not a defect in the book; not fixed because there is nothing to fix.
>
> Net result: the diagram redraw work from Stage 3 onward holds up under an actual browser engine with zero new defects. Scratch screenshots and driver script live outside the repo (temporary session workspace) and were not committed.
>
> **Calculation-detail pass** — every formula in the book (Ohm's law, RC/LC time constants, voltage-divider/Thévenin, Boolean/NAND derivations, clock/instruction-rate math, register bit-shifts, timer/prescaler math, PWM/ADC formulas, UART bit-timing, I2C address-shifting, antenna wavelength, two-point calibration, EMA smoothing, deep-sleep microsecond math) was reviewed door by door and expanded so the arithmetic path from formula to final answer is fully visible instead of jumping straight to the result — e.g. Door 1's "R = 3V ÷ 0.015A = 200Ω" now shows the mA→A conversion and the division itself; Door 9's `(1<<5)` bit-shift is now walked out bit-by-bit in binary; Door 16's calibration and EMA examples now include one full worked numeric pass through the actual constants used in the firmware. No previously-verified final numeric answer was changed — this pass only made each derivation's intermediate steps explicit, confirmed by re-checking every guardrail value (Door 3's forced-β≈38, Door 4's 10-minute/83× figures, Door 6's 3kΩ soil-probe value, Door 7's Shannon-21, Door 11's 0.5Hz toggle figure, Door 14's ARM vector-table note, Door 15's OASIS-2014/ISO-2016 distinction) after editing. `node --check` clean on all four files; `audit.py` unchanged (still only the 9 confirmed-intentional structural deviations). No `<svg>` diagram content was touched.

> **Bilingual naturalness pass** — the book's Bengali is the primary composition (idiomatic, culturally embedded); the English `dialogue en`/`scene-setting en`/`aen` blocks are a companion translation. Four parallel passes (one per door file) read every paired Bengali/English block hunting for English that reads as a stiff, word-by-word carry-over of Bengali sentence structure rather than natural prose — literal idioms/proverbs, dangling clauses, wrong-register word choices, and outright meaning shifts. ~40 spots fixed across all 16 doors, all in the English side (the Bengali prose was uniformly natural — no back-translation artifacts found). Highlights: Door 4's "column-ruined ledger" was nonsense English for "ruled into columns"; Door 4's "spendthrift... mendicant" pairing used the wrong word (মেয়াদি yatnobaan is "careful," not "a beggar"); Door 8's English dialogue was **missing an entire paragraph** — the Bengali's Arduino-Uno-vs-ESP32 "transparent chip / field chip" comparison had no English counterpart at all and has now been added; Door 14 had a typo ("Pilims" → "Pilgrims"); Door 15's cannon-teaching-metaphor callback to Book 55 had been mistranslated as "rudder-turns" and then switched to an unrelated "keel" image mid-sentence, breaking its own metaphor — restored to the cannon image throughout; Door 16's closing line asked the reader to write "trustworthy" (an adjective) next to their name where the Bengali আমানতদার is a noun/title — changed to "Trustee." Full itemized list of all ~40 fixes is in this session's edit history; not reproduced here for length. `node --check` clean on all four files; `audit.py` unchanged (still only the 9 confirmed-intentional structural deviations).
>
> **2026-08-20 full re-review + improvement pass** — fresh verification of everything above plus two new commits' worth of changes, then a targeted improvement round. Findings fixed: (1) `SAVE_KEY` was still `knowledgeCraftsman_v2` — copy-paste from the template, shared by THREE books (knowledge-craftsman, reinforcement-learning, silicon-workshop): one book's localStorage progress silently overwrote another's. Flagged in playbook §3.7 during the first review, never applied. Now `siliconWorkshop_v1` / `reinforcementLearning_v1` (Book 55's key fixed as a bonus). (2) Door 9 typo যদু→জাদু ×2 (the central "LED glow is magic" metaphor was misspelled; caught by fresh-eyes reader). Re-verified after the two unreviewed commits: all ~30 new calculations hand-checked correct, all 7 guardrail values intact, BN/EN pair balance perfect (16/16 scenes, 46/46 dialogues), search index present, live app renders. Improvements (all real-Chrome bbox-verified, zero overflow): Door 8 +level-shifting code-block (5V↔3.3V divider math 10k/20k→3.3V, TXB0108, BSS138, 0.6×VCC rule — the practical gap at the two-chip seam); Door 11 +❌/✅ compare cards for the millis()-rollover idiom; Door 13 +new SVG "SPI-র চার মোড" (CPOL/CPHA four-quadrant waveforms — was the only diagram-shaped gap: UART frame, I²C transaction, ADC staircase all had figures, SPI modes had none); Door 14 +.text/.rodata/.data/.bss/stack/heap kv-table (6 rows, project-concrete). Book now 56 diagrams. `kv-table` and `compare` components were present in style.css since the template clone but never used — now each has its model use. audit.py now reports 11 intentional deviations (added: D13 diagram=5, D8 code-block=2).

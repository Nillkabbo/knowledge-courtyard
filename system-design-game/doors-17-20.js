...
<div class="dialogue en">"Sabr — patience, steadfastness, firmness in adversity. Allah says — 'Indeed, Allah is with the patient.' (2:153). Sabr isn't just enduring — it's making the right decision in crisis. Sending repeated requests to a crashed service = impatience. Stopping, giving fallback, waiting for recovery = sabr. Circuit Breaker is sabr in code — stop, wait, then carefully resume."</div><div class="callout warn" data-door="17"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> Database choice failure: transactional ডেটায় NoSQL — ACID নেই, টাকা হারালো। SQL = ACID, NoSQL = scale।</div></div>

<div class="secret-box">🛡️ SQL = ACID, structured। NoSQL = scale, flexible। Use case বুঝে choose করো।</div>
`,
  senior:{
...
<div class="dialogue en">"Mizan — the scale, measurement. Allah says — 'We established the scale.' (55:7). Everything has a measure. To run a city you measure — how many people, how much food, how much security. Observability is that mizan — every request, every service, every error — all measured, recorded. Not blind — with sight."</div><div class="callout warn" data-door="18"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> Index failure: query ৫ মিনিট — full table scan। Index দিলে ৫ms। কিন্তু over-index → write ধীর।</div></div>

<div class="secret-box">🛡️ Index = read দ্রুত, write ধীর। B-Tree (range), Hash (exact)।</div>
`,
  senior:{
...
<div class="dialogue en">"Tawfiq — successful completion, divine aid. Allah says — 'Whoever fears Allah, He makes a way out for him.' (65:2). Making a way out — solving a hard problem step by step. Designing a URL shortener is finding a path — applying prior knowledge at each decision. Tawfiq is the moment all steps form one complete design. The next capstone is harder — but the framework is the same."</div><div class="callout warn" data-door="19"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> Replication failure: single leader down → ৩০ সেকেন্ড downtime। Election ছিল না। Leaderless = high availability।</div></div>

<div class="secret-box">🛡️ Replication: leader-based (consistency), leaderless (availability)। CAP theorem।</div>
`,
  senior:{
...
<div class="diagram">
  ...
  <svg viewBox="0 0 560 280" xmlns="http://www.w3.org/2000/svg">
    ...
    ${[0,1,2,3,4].map(i=>`<line class="edge-hot" x1="420" y1="85" x2="${355+i*35}" y2="115" marker-end="url(#arrF20)"/>`).join('')}
    ${[0,1,2,3,4].map(i=>`<line class="edge-hot" x1="${355+i*35}" y1="139" x2="420" y2="170" marker-end="url(#arrF20)"/>`).join('')}

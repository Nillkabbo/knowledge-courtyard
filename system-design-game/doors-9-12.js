...
<div class="dialogue en">"The compilation of the Quran happened this way. First scattered — on bone, leaf, memory. Then structured — surah order, ayah order. Then verified — each ayah cross-checked against two witnesses' memorization. Then distributed — multiple copies to different regions. SQL = structure (surah order). ACID = verification (witnesses). Replication = distribution (copies). Database design isn't new — it's the ancient wisdom of preserving knowledge."</div><div class="callout warn" data-door="9"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> Circuit breaker failure: downstream fail করলো, caller retry → thread pool exhausted → cascade fail। Breaker = fail fast, recover slow।</div></div>

<div class="secret-box">🛡️ Circuit breaker = fail fast। downstream down → দ্রুত ফেরো। Recover হলে আবার খোলো।</div>
`,
  senior:{
...
<div class="dialogue en">"Fihrist — index, the map of quick finding. When the Quran was revealed, the companions compiled it — but could it be found without an index? In Islamic tradition, the collection of ilm is always organized — by chapter, letter, topic. Without an index, ilm is chaos. The database too — without an index, slow, chaotic, blind."</div><div class="callout warn" data-door="10"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> Monitoring failure: একটা API ৩ দিন error দিচ্ছিল। কেউ জানতো না — monitoring ছিল না। Observability = কান ও চোখ।</div></div>

<div class="secret-box">🛡️ Observability = Metrics + Logs + Traces। অন্ধ হয়ে prod চালিও না।</div>
`,
  senior:{
...
<div class="dialogue en">"Qibla — direction. In prayer, one qibla — toward the Kaaba. No one faces north and east simultaneously. You choose one direction. CAP Theorem is the same — you want both Consistency and Availability. But when partition (network split) happens — one must be sacrificed. Which — that's your qibla. Your system's character lies in that decision."</div><div class="callout warn" data-door="12"><span class="co-icon">⚠️</span><div><strong>বাস্তব বিপদ:</strong> News feed failure: প্রতি refresh-এ timeline হিসাব — ৫০০ms। Fan-out on read = ধীর। Fan-out on write = দ্রুত।</div></div>

<div class="secret-box">🛡️ Fan-out on write = pre-compute = দ্রুত। Fan-out on read = ধীর।</div>
`,
  senior:{

...
const SAVE_KEY = 'systemDesignCodex_v2';   // v2: 20-door complete mastery (bumped from v1)
const MAX_XP = 2000;
const RANKS = [
  {min:0,    name:'গ্রাম কারিগর',        icon:'🛖'},
  {min:100,  name:'রাস্তা নির্মাতা',      icon:'🛤️'},
  {min:200,  name:'দুর্গ প্রহরী',        icon:'🏰'},
  {min:300,  name:'শহর পরিকল্পক',        icon:'📐'},
  {min:400,  name:'স্থপতি',              icon:'🏛️'},
  {min:500,  name:'নগর নির্মাতা',        icon:'🌆'},
  {min:700,  name:'মহানগর স্থপতি',       icon:'🌇'},
  {min:900,  name:'নগরী নির্মাতা',       icon:'👑'},
  {min:1100, name:'বিশ্বব্যাপী বিতরণকারী', icon:'🌍'},
  {min:1300, name:'কাউন্সিল প্রধান',      icon:'🧭'},
  {min:1500, name:'প্রধান স্থপতি',        icon:'🏛️'},
  {min:1700, name:'নগর দর্শনীয়',         icon:'🌟'},
  {min:1900, name:'চিরস্থায়ী নগর নির্মাতা', icon:'✨'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=35+Math.random()*25;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

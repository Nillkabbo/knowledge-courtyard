...
const RANKS = [
  ...
  {min:300,  name:'ফাংশন কারিগর',           icon:'🔧'},
  {min:500,  name:'রিফ্যাক্টরার',           icon:'♻️'},
  {min:700,  name:'প্যাটার্ন নকশাবিদ',      icon:'🏗️'},
  {min:900,  name:'প্র্যাগম্যাটিস্ট',        icon:'🎯'},
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=225+Math.random()*30;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

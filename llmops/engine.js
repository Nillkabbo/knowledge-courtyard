// ════════════════════════════════════════
// Book 14 · LLMOps — Engine
// ════════════════════════════════════════
let state = { xp:0, currentDoor:null, completedDoors:[], prologueSeen:false };
const XP_PER_DOOR = 100;
const SAVE_KEY = 'llmops_v1';
const MAX_XP = 1000;
const RANKS = [
  {min:0,   name:'পরিচালক',     icon:'🚀'},
  {min:100, name:'প্রযুক্তিবিদ',   icon:'📦'},
  {min:200, name:'অভিজ্ঞ প্রযুক্তিবিদ',     icon:'📊'},
  {min:300, name:'প্রধান প্রযুক্তিবিদ',     icon:'🔍'},
  {min:400, name:'নিয়ন্ত্রণ স্থপতি',           icon:'⚙️'},
  {min:500, name:'পরিচালনা স্থপতি',     icon:'💰'},
  {min:600, name:'নিয়ন্ত্রণ স্থপতি',    icon:'🏙️'},
  {min:700, name:'নিয়ন্ত্রণ সুলতান',    icon:'👑'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=180+Math.random()*30;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

// ════════════════════════════════════════
// Book 6 · Interview Preparation — Engine
// ════════════════════════════════════════
let state = { xp:0, currentDoor:null, completedDoors:[], prologueSeen:false };
const XP_PER_DOOR = 100;
const SAVE_KEY = 'interviewPrep_v1';
const MAX_XP = 800;
const RANKS = [
  {min:0,   name:'প্রতিযোগী',     icon:'⚔️'},
  {min:100, name:'চ্যালেঞ্জার',   icon:'🛡️'},
  {min:200, name:'ফাইনালিস্ট',     icon:'👑'},
  {min:300, name:'চ্যাম্পিয়ন',     icon:'🏅'},
  {min:400, name:'বিজয়ী',           icon:'🏆'},
  {min:500, name:'প্রতিযোগিতার সেরা',     icon:'🎖️'},
  {min:600, name:'বিজয়ী',    icon:'💎'},
  {min:700, name:'চূড়ান্ত বিজয়ী',    icon:'👑'}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=220+Math.random()*30;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}

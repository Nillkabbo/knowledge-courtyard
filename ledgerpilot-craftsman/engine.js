...
const SAVE_KEY = 'ledgerPilotCraftsman_v5';
const MAX_XP = 3700;
const RANKS = [
  ...
  {min:700,  name:'নথির কারিগর',      icon:'📜'},
  {min:800,  name:'তালার কারিগর',     icon:'🔐'},
  {min:1000, name:'চুক্তির কারিগর',   icon:'🪞'},
  {min:1200, name:'তুলাদণ্ডের কারিগর', icon:'⚖️'},
  {min:1400, name:'শহরের কারিগর',     icon:'🏙️'},
  {min:1600, name:'হিকমাহ অধিকারী',      icon:'👑'},
  {min:1800, name:'স্মৃতি কারিগর',      icon:'🧠'},
  {min:2400, name:"রন্ধনপ্রণালীর কারিগর", icon:"📜"},
  {min:2700, name:"ভিত্তি-স্থপতি", icon:"🏛️"},
  {min:3000, name:"খনির কারিগর", icon:"⛏️"},
  {min:3600, name:"নিবন্ধ-রক্ষক", icon:"📋"},
  {min:3700, name:"অধিকারী হিসাবের কারিগর", icon:"🌟"}
];
...
class Particle{constructor(){this.reset();this.y=Math.random()*H}reset(){this.x=Math.random()*W;this.y=H+10;this.size=Math.random()*2+.5;this.speedY=Math.random()*.35+.12;this.speedX=(Math.random()-.5)*.15;this.opacity=Math.random()*.45+.12;this.hue=160+Math.random()*25;this.twinkle=Math.random()*6.28}update(){this.y-=this.speedY;this.x+=this.speedX;this.twinkle+=.025;if(this.y<-10)this.reset()}draw(){const f=Math.sin(this.twinkle)*.3+.7;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,6.28);ctx.fillStyle=`hsla(${this.hue},65%,58%,${this.opacity*f})`;ctx.shadowBlur=5;ctx.shadowColor=`hsla(${this.hue},65%,58%,${this.opacity*.4})`;ctx.fill()}}
...
// Map
function renderMap(){const grid=document.getElementById('doors-grid');grid.innerHTML='';const SECTIONS=[[1,3,'অংশ ০ — ভিত্তি','Foundation · BE stack, FE stack, tooling & deploy','#94a3b8'],[4,8,'অংশ I — শহর','The City · domain, clients, timesheets, inventory, FSM','#2dd4bf'],[9,13,'অংশ II — টাকার মেঝে','The Money Floor · min-check, credits, NACHA, Xero, cycles','#818cf8'],[14,20,'অংশ III — কারুকাজ','The Craft · dry-run, guards, RBAC, emit, force-import, contract, manifest','#fbbf24'],[21,23,'অংশ IV — শৃঙ্খলা','Discipline · FE, BE, quality gates','#34d399'],[24,29,'অংশ V — গভীর খনি','The Deep Mine · ORM, DRF, MySQL, Celery, Redis, Vue reactivity','#22d3ee'],[30,36,'অংশ VI — রন্ধনপ্রণালী ও সমাপ্তি','Recipes & Synthesis · feature map, charter, selector, infra, testing, first week, caravan','#f472b6'],
[37,37,'পরিশিষ্ট','Appendix · the screen register — all 42 views','#c084fc']];let nextIdx=-1;doors.forEach((door,idx)=>{const sec=SECTIONS.find(s=>door.num>=s[0]&&door.num<=s[1]);if(sec&&door.num===sec[0]){const h=document.createElement('div');h.className='map-section-label';h.style.cssText=`grid-column:1/-1;margin:1.1rem 0 .3rem;padding:.55rem .9rem;border-left:3px solid ${sec[4]};background:rgba(148,163,184,.06);border-radius:0 10px 10px 0;font-size:.78rem;letter-spacing:.04em;color:${sec[4]};font-weight:700`;const secDone=doors.filter((d,i)=>d.num>=sec[0]&&d.num<=sec[1]&&state.completedDoors.includes(i)).length;const secTotal=sec[1]-sec[0]+1;h.style.setProperty('--sec',sec[4]);h.innerHTML=`${sec[2]} <span class="sec-sub">${sec[3]}</span><span class="sec-progress ${secDone===secTotal?'done':''}">${secDone}/${secTotal}${secDone===secTotal?' ✓':''}</span>`;grid.appendChild(h)}const prevDone=state.completedDoors.includes(idx-1);const selfDone=state.completedDoors.includes(idx);const unlocked=idx===0||prevDone||selfDone;if(unlocked&&!selfDone&&nextIdx===-1)nextIdx=idx;const card=document.createElement('div');card.className=`door-card ${selfDone?'completed':''} ${!unlocked?'locked':''}`;if(unlocked){card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-label',`${door.name} — ${door.subtitle}`);card.onclick=()=>openDoor(idx);card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openDoor(idx)}}}else{card.setAttribute('aria-label',`${door.name} — locked`)}card.innerHTML=`<div class="door-head"><span class="door-num">${door.num}</span><span class="door-icon">${door.icon}</span></div><div class="door-title">${door.name}</div><div class="door-subtitle">${door.subtitle}</div><div class="door-tech">${door.tech}</div>${!unlocked?'<div class="door-lock-hint">🔒 আগের দরজা সম্পন্ন করো</div>':''}`;grid.appendChild(card)});if(nextIdx>=0){const cards=grid.querySelectorAll('.door-card');if(cards[nextIdx]){cards[nextIdx].classList.add('next-door');cards[nextIdx].scrollIntoView({block:'center',behavior:'smooth'})}}updateHUD()}
function updateHUD(){const pct=Math.min(100,(state.xp/MAX_XP)*100);document.getElementById('hud-xp-fill').style.width=pct+'%';document.getElementById('hud-xp-text').textContent=`${state.xp} / ${MAX_XP} XP`;let rank=RANKS[0];for(let i=RANKS.length-1;i>=0;i--){if(state.xp>=RANKS[i].min){rank=RANKS[i];break}}document.getElementById('hud-rank-icon').textContent=rank.icon;document.getElementById('hud-rank-name').textContent=rank.name;document.getElementById('hud-level').textContent=`স্তর ${Math.min(37,Math.floor(state.xp/100)+1)}`}
// Cheat Modal
...
function fontStep(d){fontScale=Math.max(80,Math.min(140,fontScale+d*10));try{localStorage.setItem('lp_font_scale',String(fontScale))}catch(e){}applyFontScale()}
...
window.addEventListener('scroll',()=>{const f=document.getElementById('read-fill');if(!f)return;const h=document.documentElement;const p=h.scrollTop/(h.scrollHeight-h.clientHeight)*100;f.style.width=Math.min(100,Math.max(0,p))+'%';const t=document.getElementById('to-top');if(t)t.style.opacity=h.scrollTop>400?'1':'0'},{passive:true});
...
let fontScale=100;try{fontScale=parseInt(localStorage.getItem('lp_font_scale'))||100}catch(e){}
applyFontScale();
//
